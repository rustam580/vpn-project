...
- Status: in_progress
- Problem: WebRTC rescue is now an `olcrtc` productization track, not a custom transport-build track. The primary profile is `wbstream + vp8channel`; the old Python WebRTC gateway experiment was removed on 2026-05-18 to avoid maintaining a slower parallel implementation. On 2026-05-18, the Rescue Beta path passed LDPlayer whole-device browser validation and one real mobile field check in Anapa, RU during a whitelist-like restriction. Remaining risks: carrier fragility, room lifecycle, sidecar deployment/client distribution, route/auth policy, account risk, support burden, and legal/product positioning.
- Next action: convert the successful manual Rescue Beta into a repeatable closed-beta package: branded APK build artifact, non-technical install/import instructions, room/key rotation runbook, and a 7-day field test matrix across at least 3 operators/regions. Guest WB room creation returned `Guests are not allowed to create room`, so automation now uses a pre-created warm room pool: operators create WB rooms, `/rescue_room_add` stores them, `/rescue_room_warm` starts a systemd-managed `olcrtc` relay before assignment, and `/rescue_create` prefers warm rooms for instant user delivery. Keep isolated from payments, Marzban, and public tariffs until closed beta criteria in `docs/webrtc-transport-research.md` are met.
- Owner: dev/research

## Recently Closed
- Validated the first real Rescue Beta field sample: `wbstream + vp8channel` via Olcbox loaded previously unavailable mobile-data apps/pages during an active whitelist-like restriction in Anapa, RU. LDPlayer lab checks also showed Chrome egress through VPS `104.238.29.239` and Speedtest around `6.19 Mbps` download / `404 ms` ping.
- Added the first repeatable Rescue Beta packaging pieces: `scripts/generate_olcrtc_rescue_configs.py --create-wb-room` attempt path, optional URI `client_id`, `session.json`/`room-url.txt` artifacts, and a `systemd` service template for one managed `olcrtc` server session per room.
- Added warm Rescue room pool operations: DB metadata for warm room keys/URIs, `/rescue_room_warm`, `/rescue_create` warm-room reuse, `/rescue_rooms` key visibility, and admin keyboard/help entries.
- Removed the old Python WebRTC gateway experiment and chose the focused Rescue Beta implementation path: isolated `olcrtc` Go/Pion sidecar with `wbstream + vp8channel`, plus RootVPN-owned packaging, session management, bot UX, and operations.
- Added `experiments/olcrtc-rescue/README.md` plus `scripts/generate_olcrtc_rescue_configs.py` to produce server/client YAML and `olcrtc://` URI with WB URL-to-room-ID normalization.
- Reviewed `olcrtc` `vp8channel`: it sends valid-looking VP8 samples with binding/epoch headers and KCP payloads through Go/Pion `TrackLocalStaticSample`; pure Python is uncertain unless the LiveKit/aiortc stack can publish encoded VP8 samples directly, so a Go/Pion sidecar or olcRTC backend spike is the pragmatic fast-path candidate.
