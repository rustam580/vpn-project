...
- Status: in_progress
- Problem: WebRTC/DataChannel may be useful as a reserve transport, but whitelist-resilience requires a carrier layer through already-allowed video/conference services, not only a self-hosted gateway. WB Stream probing works for guest join/token retrieval, but guest LiveKit data packets are blocked in the tested room (`can_publish_data=false`). Synthetic video-track delivery, one-frame encrypted byte delivery, multi-frame one-way delivery, reverse video ACK, sliding-window retry, stream-mode byte reassembly, and tuning sweeps work, so the likely WB route is media-frame encoding. Local SOCKS shape now exists as a loopback-only fake-egress harness, proxy bundles can be delivered over WB stream-mode in lab form, and `remote_proxy_endpoint.py` now decodes `RPB1` request bundles into route-checked fake-egress response bundles. Strategic path is now `olcrtc` sidecar first, with `wbstream + vp8channel` as the primary Rescue Beta profile and Python `tile2` retained as lab fallback. Remaining risks: carrier fragility, sidecar deployment/client distribution, route/auth policy, account risk, support burden, and legal/product positioning.
- Next action: run a real `olcrtc` sidecar smoke using generated `wbstream + vp8channel` server/client configs: server on a test VPS, client locally with SOCKS `127.0.0.1:8808`, then measure `curl --socks5-hostname` success, latency, throughput, reconnect, and room/account TTL. Keep isolated from payments, Marzban, and public tariffs until closed beta criteria in `docs/webrtc-transport-research.md` are met.
- Owner: dev/research

## Recently Closed
- Chose the Rescue Beta implementation path: prefer isolated `olcrtc` Go/Pion sidecar with `wbstream + vp8channel` over extending the Python `tile2` carrier for product testing. Added `experiments/olcrtc-rescue/README.md` plus `scripts/generate_olcrtc_rescue_configs.py` to produce server/client YAML and `olcrtc://` URI with WB URL-to-room-ID normalization.
- Added `remote_proxy_endpoint.py`, the server-side lab protocol half for WebRTC proxy bundles: decode `RPB1`, enforce explicit host:port route policy, run injected fake egress, and return `RPB1` response bundles without dialing the network itself.
...
- Added `proxy_packet_bundle.py` and `wbstream_proxy_carrier.py`: internal proxy packets are bundled as `RPB1` payloads and can be delivered through `wbstream_video_window_probe(..., stream_mode=True)` in lab mode. The carrier is delivery-only until a remote endpoint can return response bundles.
...
- Added explicit `data_repeats` redundancy knob to the sliding-window and tuning-sweep probes. It repeats due data chunks without counting duplicates as timeout retransmits.
