...
- Next action: use `tile2,data_repeats=1` as the current R&D baseline candidate, run larger repeated sweeps (`2048` bytes, window/FPS variants), then improve codec resilience (tile redundancy / QR-style recovery inspired by olcRTC); keep isolated from payments, Marzban, and public tariffs until closed beta criteria in `docs/webrtc-transport-research.md` are met.
- Owner: dev/research
