...
- WebRTC/DataChannel transport is documented as an R&D track only (`docs/webrtc-transport-research.md`); do not sell or advertise it until a closed beta proves stability and cost. As of 2026-05-18, the product path is an isolated `olcrtc` Go/Pion sidecar using `wbstream + vp8channel`; the older Python WebRTC gateway experiment was removed to avoid maintaining a slower parallel implementation. A first real mobile field check in Anapa, RU validated the hypothesis for one sample: non-whitelisted mobile-data apps/pages failed before Olcbox and loaded after starting the Rescue Beta URI.
...
- WebRTC Rescue Beta: `experiments/olcrtc-rescue/` contains the active implementation path. `scripts/generate_olcrtc_rescue_configs.py` generates `wbstream + vp8channel` server/client YAML and `olcrtc://` URI, normalizing WB room URLs to room IDs for upstream compatibility. It now also writes `session.json`/`room-url.txt`, supports optional URI `client_id`, and includes a `systemd` template for one managed `olcrtc` server session per room. Lab validation routed LDPlayer/Chrome through test VPS `104.238.29.239` with Speedtest around `6.19 Mbps` download and `404 ms` ping, and one real mobile field check in Anapa, RU loaded previously unavailable mobile-data apps/pages during a whitelist-like restriction. Guest WB room creation can fail with `Guests are not allowed to create room`, so product automation uses a pre-created room pool: `/rescue_room_add` stores rooms, `/rescue_room_warm` starts the server relay before assignment, and `/rescue_create` prefers warm rooms for instant user delivery. The weakest observed layer is provider/session setup reliability, room lifecycle, and client packaging, not `vp8channel` throughput after both participants join.
- SQLite schema version latest: `6`.
