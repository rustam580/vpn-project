from config import _parse_plans_json


def test_parse_plans_json_defaults_to_single_plan() -> None:
    plans = _parse_plans_json("", default_days=30, default_gb=0, default_rub=99.0)
    assert len(plans) == 1
...
def test_parse_plans_json_accepts_custom_plans() -> None:
    ...
    plans = _parse_plans_json(raw, default_days=30, default_gb=0, default_rub=99.0)
