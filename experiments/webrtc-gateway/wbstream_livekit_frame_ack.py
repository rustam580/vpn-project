...
DEFAULT_MESSAGE = "RootVPN ACK video carrier " * 40


@dataclass(frozen=True)
class VideoAckResult:
    ok: bool
    room_id: str
    server_url: str
    elapsed_ms: int
...
async def wbstream_video_ack_probe(
    room: str,
    *,
    ...
    details_a, details_b = await asyncio.gather(
        probe_room(room_id, display_name="RootVPN ACK A", timeout_sec=timeout_sec),
        probe_room(room_id, display_name="RootVPN ACK B", timeout_sec=timeout_sec),
    )
    ...
    async def process_ack_stream(stream: rtc.VideoStream) -> None:
        ...
        async for event in stream:
            ...
            if ack.total != total_chunks:
                continue
            acked_seqs.update(ack.received)
            if len(acked_seqs) == total_chunks and not all_acked.done():
    ...
    try:
        ...
        data_track = rtc.LocalVideoTrack.create_video_track("rootvpn-frame-data", data_source)
        ack_source = rtc.VideoSource(width=DEFAULT_WIDTH, height=DEFAULT_HEIGHT)
        ack_track = rtc.LocalVideoTrack.create_video_track("rootvpn-frame-ack", ack_source)
        ...
        async def push_data_frames() -> None:
            ...
            while not all_acked.done():
                pending = [idx for idx in range(total_chunks) if idx not in acked_seqs]
                if not pending:
...
async def _amain() -> int:
    parser = argparse.ArgumentParser(description="Send WB Stream video payload with reverse video ACKs")
    ...
    result = await wbstream_video_ack_probe(
        args.room,
